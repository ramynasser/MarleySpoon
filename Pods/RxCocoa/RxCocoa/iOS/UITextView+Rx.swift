...
//
    ...
    extension Reactive where Base: UITextView {
        ...
        public var didBeginEditing: ControlEvent<()> {
            return ControlEvent<()>(events: delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidBeginEditing(_:)))
                .map { _ in
                    ()
            })
        }
        ...
        public var didEndEditing: ControlEvent<()> {
            return ControlEvent<()>(events: delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidEndEditing(_:)))
                .map { _ in
                    ()
            })
        }
        ...
        public var didChange: ControlEvent<()> {
            return ControlEvent<()>(events: delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChange(_:)))
                .map { _ in
                    ()
            })
        }
        ...
        public var didChangeSelection: ControlEvent<()> {
            return ControlEvent<()>(events: delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChangeSelection(_:)))
                .map { _ in
                    ()
            })
        }
